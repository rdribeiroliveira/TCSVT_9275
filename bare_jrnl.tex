...
\textcolor{red}{Falta falar de seguranca ... e fale que o FPGA deve prever algoritmos mais complexos que o atual... mais poder de processamento... deixe tudo em azul}

\subsection{Discussion about the Proposed Methodology}\label{system-compatibility}

\textcolor{blue}{The proposed methodology provides the transmission of pre-synthesized hardware cores, addressing FPGAs integrated into commercial receivers. Besides, the same content-transmission strategy, which is already used for general data and SI ({\it e.g.} tables, etc.), is employed.}

\textcolor{blue}{As one may notice, the presented reconfiguration system, at the receiver, is based on JTAG communication, SVF files, which are retrieved from the digital TV signal, and a newly created SI table (UIT) and its associated descriptors (see section \ref{data-multiplexing}), which are in charge of providing information regarding the hardware reconfiguration content. The SVF format is a standard used by almost all FPGA manufacturers (\textcolor{red}{isso é verdade?}) and is compliant with the JTAG communication protocol. Besides, the proposed SI extension carries all necessary information regarding the FPGA device ({\it e.g.} manufacturer, part number, family identification, etc.) under update, which allows the complete identification of a specific unit. As a consequence, it is not restricted to a specific manufacturer or FPGA model and is flexible enough in order to be adapted to other scenarios. For example, nearly any format could be sent in private sections, as long as suitable modifications are done in the target receiver system, considering that related content is seen as a regular data flow by the data broadcast mechanisms.}

\textcolor{blue}{Regarding content reception, the pre-synthesized core will be accepted only if there is a match between the information transmitted, which was filtered from UIT, and the locally available FPGA device; otherwise, the receiving system simply ignores the acquired content and waits for a suitable content.}
...
\textcolor{blue}{The experimental setup used in this work include a set of tools necessary to validate all steps of this methodology. To synthesize the digital circuits was used a tool Quartus II v.11.0 provided by the Altera FPGA manufacturer. This tool generate the programmer file in several formats including the universal format, SVF. To multiplex the content on a transport stream, was used the Advanced Stream Combiner (ASC) tool provided by the manufacturer Rohde \& Schwarz. During the creation of the transport stream was add a new table (UIT) and the SVF content was split and multiplexed in private sections of 4080 bytes. To modulation and broadcast was used a VHF/UHF Modulator (DekTec DTU-215, USB-2 device) with a player tool StreamXpress v.3.10.2 also provided by DekTec company. The receiver device used for test and development was a NXP-STB225 IP and hybrid DTV set-top box provide by NXP semiconductors company. The reference design is a Linux OS based system and uses a dual core 300 MHz MIPS processor. Lastly, for the FPGA device was used a Nios Stratix EP1S10F780C6 development board.}
...
\textcolor{blue}{In this work were not performed tests with the video decoders to measure the behavior of the system. The overhead presented by the methodology is largely affected by the pre-synthesized file format. Universal formats, such as, SVF include more information on final file increasing the size of the file. Consequently, the remounting process will be slower. After this, is necessary to parser the file to extract the SVF syntax before to reconfigure the FPGA device causing more overhead at the system. Whereas using proprietary formats the overhead is decreased and the complexity is transferred to the programmer device. Remembering that using proprietary formats is also necessary to implement dedicated reconfiguration schemes.}\textcolor{red}{Aqui comente novamente sobre o overhead da metodologia}
