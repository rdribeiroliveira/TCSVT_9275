...
\textcolor{blue}{This is a hybrid architecture that uses a CPU and a reconfigurable device (FPGA) to performing tasks. The system is controlled by a resource manager (RM) that uses a decision algorithm to choose between, pre-synthesized hardware or software modules. The hardware and software modules are keep in a resource queue. When the RM opting by the use of a hardware module the FPGA (is reconfigured) performs the task or RM choose a software module, CPU performs the task.}
...
\begin{figure}[ht]
\centering
\includegraphics[width=3.0in]{images/Fig3.eps}
...
\begin{figure}[ht]
\centering
\includegraphics[width=3.5in]{images/bigpicture.eps}
\caption{All steps envolved with the scheme.}
