...
Intelligent embedded systems can make use of the partial dynamic hardware, for run-time reconfiguration. Such a feature, which is similar to a microprocessor multi-task system, allows a multiplex of distinct hardware modules to run at the same time. Thus, functional pre-synthesized hardware blocks, {\em i.e.}, logical blocks, can be reconfigured or not, according to system needs~\cite{ref14}. \textcolor{blue}{The system architecture are controlled by a Microblaze device that is in charge to act with the system and reconfigure pre-synthesized hardware blocks at the FPGA. The list of pre-synthesized hardware blocks are kept on a flash memory.} In that case, the use of partial dynamic reconfiguration causes a considerable reduction in power consumption, besides a significant decrease in device costs.
