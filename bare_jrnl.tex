...
\textcolor{blue}{The scheme proposed in this work allows broadcast pre-synthesized hardware for different models of FPGA. For this is used the same digital TV strategy that is content signalization through tables. The UIT table has the syntax necessary to describe the hardware signal flow that are multiplexed at the signal. Thus is possible to broadcast pre-synthesized hardware for any model of FPGA. For this are added some crucial FPGA information, such as, FPGA manufacturer name, FPGA Family name, FPGA part number, FPGA core size, etc. Thus, the receiver units are able to extract the field contents of UIT table to compare with the FPGA model information fitted on receiver architecture. The pre-synthesized content will be accepted, only, if the FPGA model details filtered from UIT table correspond with the FPGA of receiver. With this, the receivers select the content broadcasted for their unit. If the UIT table content not match with the model of FPGA of the receiver, the system simply ignores this content. When the content is identified by the receiver system is started the reassembling of the private sections loading the bitstream content. In this process, for safety reason, the content of each retrieved section is made the checksum with their respective CRC value. If the content is not corrupted it is used on remounting process. If for some reason it is corrupted, it is ignored and the system will wait for another section in the next cycle. At the end of this process, the pre-synthesized SVF file is remounted at the receiver file system. After this, the system start the reconfiguration process. The overhead with this process is associated with the time to parser of SVF file format and the JTAG comunication. In proprietary formats this overhead is smaller, however, it would be necessary to have a dedicated programming scheme for each model of FPGA. On the other hand, the SVF format is standard used by all FPGA manufacturer and it is compliance with JTAG communication. The choice of the FPGA programming mode is a project decision and is related with each FPGA model and the programmer file format. The scheme proposed in this work can work with all formats, considering that content it is seen as a regular data flow by the data broadcast mechanisms.
One may notice that the proposed methodology is based on an extension of the current service information (SI) standards \textcolor{red}{aqui vai uma ref de SI}, without any modification w.r.t. current structures, since it uses a new table (UIT) and associated descriptors. Although that may sound complicated, its implementation is transparent and can even be done without a formal modification of the related standards, as a proprietary framework. Apart from that, the present approach addresses the complete update chain and provides an elegant and complete solution, which is still more feasible and simpler than isolated proprietary solutions or on-site maintenace.}
